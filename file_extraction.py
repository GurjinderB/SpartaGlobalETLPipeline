...
def extract_file_type(s3, prefix: str, files_dict: dict, file_type: str = None) -> list:
    ...
    return academy_csvs_file_names


# calls the functions to extract each file type from the two folders 'Academy' and 'Talent' into a dictionary
def extract() -> (dict, list):
    s3 = boto3.client('s3')
    files_dict = {'academy_csv': [], 'json': [], 'txt': [], 'csv': []}

    academy_csvs_file_names = extract_file_type(s3, 'Academy', files_dict)
    ...
    return files_dict, academy_csvs_file_names
